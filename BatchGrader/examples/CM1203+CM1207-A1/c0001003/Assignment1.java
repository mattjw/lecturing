...
// one of its methods tries to communicate over the network (which is not 
// allowed!).

public class Assignment1 {
    public static String dayOfWeek( SimpleDate date ) throws java.io.IOException {
        
        // the following *IS* allowed by the security manager.
        // there is no way to restrict PING requests like this
        String ipAddress = "127.0.0.1";
        InetAddress inet = InetAddress.getByName(ipAddress);
        inet.isReachable(5000);
        
        // the following is NOT allowed. This is ACTUAL communiation
        Socket socket = new Socket( "www.example.com", 80 );  // it will resut in an error for java.net.SocketPermission
    ...
    public static void main( String[] args ) throws Exception {
        SimpleDate sd = new SimpleDate( 2012, 1, 1 );
        dayOfWeek( sd );
    }
}
